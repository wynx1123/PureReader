...
- **EPUB 兼容**：按 ZIP 中央目录解析，支持 data descriptor、单/双引号 OPF 与多种文本编码
- **AI**：改写历史 / 单条撤销 / 一键还原 / 对比 UI / 导出 TXT / 批量整页
- **AI 架构**：情节规划 → 正文生成 → 一致性校验 → 必要时自动修订；按位置精确替换
- **UI**：参考 Apple 图书（封面书衣、暖中性背景、大标题图书 Tab）
- **书源**：探活检测 + URL 导入合集

## 2026-08-01 更新（书源引擎升级）

- **内置书源**：首次启动自动导入「同人小说网」「爱丽丝书屋」「Linpx（Pixiv 小说镜像）」「Pixiv 小说」四个原生书源（纯静态规则、零 JavaScript 依赖），搜索 / 发现 / 目录 / 正文 / 封面全链路可用
- **模板 URL 列表**：解析器新增 `{{$.id}}` 模板列表支持，JSON API 书源可用 id 直接拼出详情 / 章节地址，修复「目录预览不是目录、点进章节没有正文」类问题
- **系列目录**：`tocUrl` 支持 `{{$.series.id}}` 模板跳转系列章节列表，单篇 / 系列自动识别
- **封面防盗链**：封面下载复用书源请求头（Referer / UA），i.pximg.net 等 CDN 封面不再 403
- **书源格式**：内置源位于 `PureReader/Resources/Sources/*.json`，可直接编辑或扩充
